...
export const worldInfoApi = {
	...
	getWorldNews: createAsyncThunk(
		...
		async (props: { searchValue: string; sortItems: string }) => {
			...
			const response = await axios.get(
				...
				}&sortBy=${sortItems}&language=en&apikey=${process.env.REACT_APP_NEWS_API_KEY}`
			);
			return response;
		}
	),
	...
	// 공공데이터포털 외교부 (국가일반정보)
	getNationGeneralInfo: createAsyncThunk(
		'publicDataPortal/getNationGeneralInfo',
		async (code: string) => {
			const response = await axios.get(
	...
	// 공공데이터포털 외교부 (국가경제정보)
	getNationEconomyInfo: createAsyncThunk(
		'publicDataPortal/getNationEconomyInfo',
		async (code: string) => {
			const response = await axios.get(
	...
	// 공공데이터포털 외교부 (국가정치정보)
	getNationPoliticsInfo: createAsyncThunk(
		'publicDataPortal/getNationPoliticsInfo',
		async (code: string) => {
			const response = await axios.get(
